...
func NewSchema() Schema {
	fields := map[string]Field{
		...
		"publish_ts": {
			...
			Column: Column{Table: "memo", Name: "payload"},
			Expressions: map[DialectName]string{
				DialectSQLite:   "COALESCE(CAST(JSON_EXTRACT(%s, '$.publishTs') AS INTEGER), 0)",
				DialectMySQL:    "COALESCE(CAST(JSON_UNQUOTE(JSON_EXTRACT(%s, '$.publishTs')) AS SIGNED), 0)",
				DialectPostgres: "COALESCE((%s->>'publishTs')::bigint, 0)",
			},
			AllowedComparisonOps: map[ComparisonOperator]bool{
				CompareEq:  true,
				CompareNeq: true,
	...
	envOptions := []cel.EnvOption{
		...
		cel.Variable("publish_ts", cel.IntType),
