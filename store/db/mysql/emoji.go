...
func (d *DB) CreateEmojiGroup(ctx context.Context, group *store.EmojiGroup) (*store.EmojiGroup, error) {
	result, err := d.db.ExecContext(ctx, `INSERT INTO emoji_group(name) VALUES(?)`, group.Name)
	if err != nil {
		return nil, err
	}
	...
	group.ID = int32(id)
	err = d.db.QueryRowContext(ctx, `SELECT created_ts,updated_ts FROM emoji_group WHERE id=?`, group.ID).Scan(&group.CreatedTs, &group.UpdatedTs)
	return group, err
}
...
func (d *DB) CreateEmoji(ctx context.Context, emoji *store.Emoji) (*store.Emoji, error) {
	result, err := d.db.ExecContext(ctx, `INSERT INTO emoji(group_id,name,filename,type,size,storage_type,reference,storage_id,storage_key,blob)
		VALUES(?,?,?,?,?,?,?,?,?,?)`, emoji.GroupID, emoji.Name, emoji.Filename, emoji.Type, emoji.Size, emoji.StorageType, emoji.Reference,
		emoji.StorageID, emoji.StorageKey, emoji.Blob)
	if err != nil {
		return nil, err
	}
	id, err := result.LastInsertId()
	...
	emoji.ID = int32(id)
	err = d.db.QueryRowContext(ctx, `SELECT created_ts,updated_ts FROM emoji WHERE id=?`, emoji.ID).Scan(&emoji.CreatedTs, &emoji.UpdatedTs)
	return emoji, err
}
