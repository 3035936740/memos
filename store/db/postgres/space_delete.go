...
func (d *DB) DeleteSpace(ctx context.Context, delete *store.DeleteSpace) (*store.DeleteSpaceResult, error) {
	...
	if err := requirePostgresSpaceDeleteAdmin(ctx, tx, delete); err != nil {
		return nil, err
	}
	memoIDs, err := listPostgresSpaceDeleteMemoIDs(ctx, tx, delete.ID)
	if err != nil {
		return nil, errors.Wrap(err, "failed to collect assigned memos")
	}
	attachments, err := deletePostgresMemoSetTx(ctx, tx, memoIDs)
	if err != nil {
		return nil, errors.Wrap(err, "failed to delete assigned memo set")
	}
	if _, err := tx.ExecContext(ctx, "DELETE FROM space_member WHERE space_id = $1", delete.ID); err != nil {
		return nil, errors.Wrap(err, "failed to delete space memberships")
	}
	result, err := tx.ExecContext(ctx, "DELETE FROM space WHERE id = $1", delete.ID)
	if err != nil {
...
func requirePostgresSpaceDeleteAdmin(ctx context.Context, tx *sql.Tx, delete *store.DeleteSpace) error {
...
func listPostgresSpaceDeleteMemoIDs(ctx context.Context, tx *sql.Tx, spaceID int32) ([]int32, error) {
	rows, err := tx.QueryContext(ctx, "SELECT id FROM memo WHERE space_id = $1 ORDER BY id", spaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func deletePostgresMemoSetTx(ctx context.Context, tx *sql.Tx, memoIDs []int32) ([]*store.Attachment, error) {
	attachments, err := listPostgresMemoSetAttachments(ctx, tx, memoIDs)
	if err != nil {
		return nil, errors.Wrap(err, "failed to collect memo attachments")
	}
	if err := deletePostgresMemoSetShares(ctx, tx, memoIDs); err != nil {
...
func listPostgresMemoSetAttachments(ctx context.Context, tx *sql.Tx, memoIDs []int32) ([]*store.Attachment, error) {
...
func deletePostgresMemoSetShares(ctx context.Context, tx *sql.Tx, memoIDs []int32) error {
	for _, batch := range deleteUserBatches(memoIDs, deleteUserBatchSize) {
		clause, args := deleteUserInClause(1, batch)
