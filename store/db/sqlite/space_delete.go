...
func (d *DB) DeleteSpace(ctx context.Context, delete *store.DeleteSpace) (*store.DeleteSpaceResult, error) {
	...
	if err := requireSQLiteSpaceDeleteAdmin(ctx, tx, delete); err != nil {
		return nil, err
	}
	memoIDs, err := listSQLiteSpaceDeleteMemoIDs(ctx, tx, delete.ID)
	if err != nil {
		return nil, errors.Wrap(err, "failed to collect assigned memos")
	}
	attachments, err := deleteSQLiteMemoSetTx(ctx, tx, memoIDs)
	if err != nil {
...
func listSQLiteSpaceDeleteMemoIDs(ctx context.Context, tx dbExecutor, spaceID int32) ([]int32, error) {
...
func deleteSQLiteMemoSetTx(ctx context.Context, tx dbExecutor, memoIDs []int32) ([]*store.Attachment, error) {
	...
	if err := deleteSQLiteMemoSetShares(ctx, tx, memoIDs); err != nil {
...
func listSQLiteMemoSetAttachments(ctx context.Context, tx dbExecutor, memoIDs []int32) ([]*store.Attachment, error) {
...
func deleteSQLiteMemoSetShares(ctx context.Context, tx dbExecutor, memoIDs []int32) error {
	for _, batch := range deleteUserBatches(memoIDs, deleteUserBatchSize) {
		clause, args := deleteUserInClause(1, batch)
