...
func (d *DB) ListPollVotes(ctx context.Context, memoID int32) ([]*store.PollVote, error) {
	rows, err := d.db.QueryContext(ctx, "SELECT memo_id, user_id, option_id, device_id FROM poll_vote WHERE memo_id = ? ORDER BY id ASC", memoID)
	if err != nil {
...
func (d *DB) ListPollVotesByVoter(ctx context.Context, memoID, userID int32, deviceID string) ([]*store.PollVote, error) {
	rows, err := d.db.QueryContext(ctx, "SELECT memo_id, user_id, option_id, device_id FROM poll_vote WHERE memo_id = ? AND user_id = ? AND device_id = ? ORDER BY id", memoID, userID, deviceID)
	if err != nil {
...
func (d *DB) CountPollVotes(ctx context.Context, memoID int32) (map[string]int32, error) {
	rows, err := d.db.QueryContext(ctx, "SELECT option_id, COUNT(*) FROM poll_vote WHERE memo_id = ? GROUP BY option_id", memoID)
	if err != nil {
...
func (d *DB) SamplePollVoters(ctx context.Context, memoID int32, limit int) ([]*store.PollVoter, error) {
	rows, err := d.db.QueryContext(ctx, `
     ...
     JOIN (
       ...
       WHERE memo_id = ? AND user_id > 0
       GROUP BY user_id ORDER BY RANDOM() LIMIT ?
...
func (d *DB) ReplacePollVotes(ctx context.Context, replace *store.ReplacePollVotes) error {
	...
	if _, err := tx.ExecContext(ctx, "DELETE FROM poll_vote WHERE memo_id = ? AND user_id = ? AND device_id = ?", replace.MemoID, replace.UserID, replace.DeviceID); err != nil {
		return errors.Wrap(err, "failed to clear poll votes")
	}
	for _, optionID := range replace.OptionIDs {
		if _, err := tx.ExecContext(ctx, "INSERT INTO poll_vote (memo_id, user_id, option_id, device_id) VALUES (?, ?, ?, ?)", replace.MemoID, replace.UserID, optionID, replace.DeviceID); err != nil {
...
func (d *DB) ClearPollVotes(ctx context.Context, memoID int32) error {
	if _, err := d.db.ExecContext(ctx, "DELETE FROM poll_vote WHERE memo_id = ?", memoID); err != nil {
		return errors.Wrap(err, "failed to clear poll votes")
	}
	return nil
}
