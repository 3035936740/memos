...
function CreateSpaceDialog({ open, onOpenChange, onCreated, note }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent
        size="default"
        scrollContainerClassName="overflow-hidden gap-0"
        className="max-h-[calc(100dvh-1rem)] w-[calc(100%-1rem)] max-w-[calc(100%-1rem)] p-0 sm:max-h-[calc(100dvh-2rem)] sm:w-[calc(100%-2rem)] sm:max-w-[calc(100%-2rem)] sm:p-0 md:w-full md:max-w-md"
      >
        <form className="flex min-h-0 flex-1 flex-col" onSubmit={handleSubmit}>
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 pb-4 pt-4 sm:px-6 sm:pb-5 sm:pt-6">
            ...
            <div>
              ...
              {showCustomId && (
                <div id="space-custom-uid" className="mt-2 grid gap-2 rounded-md border bg-muted/30 p-3">
                  ...
                  >
                    {t(
                      !isSpaceUidValid ? "space.custom-id-invalid" : spaceUidConflict ? "space.custom-id-conflict" : "space.custom-id-help",
                    )}
                  </p>
                </div>
              )}
          ...
          <DialogFooter className="shrink-0 flex-row justify-end border-t border-border bg-background px-4 py-3 sm:px-6">
